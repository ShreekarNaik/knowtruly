...
export const SettingsPage = () => {
  ...
  const { useDemoData, setUseDemoData } = useConfigStore((state) => ({
    useDemoData: state.useDemoData,
    setUseDemoData: state.setUseDemoData
  }));
  ...
  return (
    <section className="space-y-6">
      <header>
        ...
        <p className="mt-2 text-sm text-slate-400">Manage environment flags and see current session details.</p>
      </header>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <section className="rounded-2xl border border-slate-800 bg-[#0f141c] p-6">
          ...
          <dl className="mt-4 space-y-3 text-sm text-slate-300">
            <div>
              <dt className="text-xs uppercase tracking-[0.25em] text-slate-500">User</dt>
              <dd>{user?.name}</dd>
            </div>
            <div>
            ...
            <div>
              <dt className="text-xs uppercase tracking-[0.25em] text-slate-500">Role</dt>
              <dd>{user?.role}</dd>
            </div>
          </dl>
        </section>
        ...
        <section className="rounded-2xl border border-slate-800 bg-[#0f141c] p-6">
          ...
          <p className="mt-2 text-sm text-slate-300">
            Demo data mirrors backend responses so you can navigate the UI without an active FastAPI server. Disable to call
            real endpoints.
          </p>
